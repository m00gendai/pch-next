...
async function getPageContent(){
    const getPageContent:Response = await fetch(
        'https://cms.pistolenclub-hallau.ch/api/content/items/imprint?populate=100',
        {
            headers: {
...
export default async function Impressum(){
    ...
    const headerImage: Medium = await getHeaderImage("Impressum")

    return(
        <main className="main">
            <div className={"page_title"} style={{backgroundImage: `url(${process.env.NEXT_PUBLIC_STORAGE}${headerImage.path})`}}><span className={"page_title_inner"}><h1>Impressum</h1></span></div>
